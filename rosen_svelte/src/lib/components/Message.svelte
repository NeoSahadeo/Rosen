...
<section class="fixed pb-4 px-4 max-w-sm w-full z-50 bottom-0 sm:left-full sm:-translate-x-full left-1/2 -translate-x-1/2">
    {#each $messages as message}
        ...
        (message.status >= 400 && message.status < 499) ? 'bg-red-400 text-red-950' : ''
            ...
            <button on:click={messages.remove(message.id)} class="bg-transparent ml-auto hover:bg-transparent hover:shadow-none">
                <img src="/MaterialSymbolsCloseSmallRounded.svg" alt="Close Message" class="mix-blend-difference">
            </button>
        </div>
    {/each}
</section>